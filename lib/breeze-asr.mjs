...
// A product-facing reference, not a benchmark or performance guarantee. The
// values come from the user's single MacBook Air observation recorded for the
// 0.49.1 release and are exposed so the UI and release documentation cannot
// drift into different numbers.
export const BREEZE_ASR_PERFORMANCE_REFERENCE = Object.freeze({
  baselineVideoDurationSeconds: 6360,
  baselineElapsedSeconds: 21600,
  baselineRatio: 3.4,
  hardware: 'MacBook Air Mac15,12／Apple M3／8 GB RAM／8 cores／macOS 26.5.2',
  sourceDate: '2026-08-18',
  scope: 'single-user observation; no profiler or raw audio artifact retained',
  message: '效能提示：低資源 Mac 可能需要數倍影片時長；曾有 1 小時 46 分影片耗時約 6 小時（約 3.4×）。需要較快結果時可改用內建 Whisper.cpp；此為單機觀察，不是效能保證。',
});
