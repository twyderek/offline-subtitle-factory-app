...
export function attachWhisperQuality(cues, qualitySegments, tolerance = 0.05) {
  ...
  const enriched = cues.map((cue, index) => {
    ...
    // A Small source cue may be split for readability. A null entry records
    // that no exact segment-to-cue mapping exists; keep the cue untouched so
    // other, unaffected cues can still retain their engine metrics.
    if (segment === null || segment === undefined) return cue;
  ...
  if (enriched.some((cue) => cue === null)) return { cues, matched: false, reason: 'timing-mismatch' };
  const matchedSegments = qualitySegments.filter((segment) => segment !== null && segment !== undefined);
  if (!matchedSegments.length) return { cues, matched: false, reason: 'no-exact-segments' };
  return { cues: enriched, matched: true, reason: matchedSegments.some((segment) => segment.confidence !== undefined || segment.noSpeechProbability !== undefined) ? 'engine-metrics' : 'no-quality-fields' };
}
