  ...
  for (let batchIndex = startBatchIndex; batchIndex < totalBatches; batchIndex += 1) {
    ...
    const body = {
      ...
      messages: [
        {
          role: 'system',
          content: `你是字幕校對員。${promptTemplate || MODE_INSTRUCTIONS[mode] || MODE_INSTRUCTIONS.proofread}${languagePromptInstruction(outputLanguage, mode)}不可更改 ID、順序、段落數或時間碼，不可杜撰內容。只輸出 JSON：{"cues":[{"id":原ID,"text":"優化文字","reason":"簡短原因"}]}`,
        },
        {
          role: 'user',
