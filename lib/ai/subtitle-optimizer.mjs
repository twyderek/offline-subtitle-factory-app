  ...
  for (let batchIndex = startBatchIndex; batchIndex < totalBatches; batchIndex += 1) {
    ...
    const body = {
      ...
      messages: [
        {
          role: 'system',
          content: `你是字幕校對員。${promptTemplate || MODE_INSTRUCTIONS[mode] || MODE_INSTRUCTIONS.proofread}${languagePromptInstruction(outputLanguage, mode)}${mode === 'translate' ? '每個 text 必須是該 cue 完整句子的完整翻譯，不可只翻譯第一個詞、不可摘要、不可輸出術語表條目。' : ''}不可更改 ID、順序、段落數或時間碼，不可杜撰內容。${search ? `本次是搜尋結果限定處理，搜尋詞為「${search}」；只可修改搜尋詞相關片段，搜尋詞以外的內容（包含平台代碼、專有名詞與其他文字）必須原樣保留。` : ''}繁體中文字幕應保留中文全形標點（例如「。」、「，」、「？」，不要任意改成英文半形標點）。只輸出符合請求 schema 的 JSON。text 必須使用輸入字幕的實際文字；不可輸出「優化文字」、「原ID」、「簡短原因」等佔位文字。若不需修改，text 必須保留原文。`,
        },
        {
          role: 'user',
