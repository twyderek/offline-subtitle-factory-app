...
function buildResponseFormat(config, batch) {
  if (config.capabilities?.structuredOutput === 'json-schema') {
    return buildSubtitleSchema(batch.length, batch.map((cue) => cue.id));
  }
...
const JSON_WRAPPER_FIELDS = Object.freeze(['response', 'output', 'result', 'data', 'content', 'message']);
...
function contentToTextFragments(content, fragments = [], seen = new Set()) {
  ...
  if (Array.isArray(content)) {
    for (const part of content) contentToTextFragments(part, fragments, seen);
    return fragments;
  }

  if (typeof content.text === 'string') fragments.push(content.text);
  for (const key of JSON_WRAPPER_FIELDS) {
...
function contentToText(content) {
  return contentToTextFragments(content).filter(Boolean).join('\n');
}
...
function parseCompletionContent(result) {
  ...
  if (!content.trim() && (rawContent === undefined || rawContent === null)) throw new Error('AI 未回傳字幕內容');

  let parsedJson = Boolean(rawContent && typeof rawContent === 'object' && !Array.isArray(rawContent));
  const directCues = findCueArray(rawContent);
  if (directCues) return directCues;

  const candidateFragments = typeof rawContent === 'string' ? [content] : fragments;
...
function canRepairLocalJson(error, config) {
  if (!['ollama', 'lm-studio'].includes(String(config?.provider || '').toLowerCase())) return false;
  return ['invalid_json', 'missing_cues'].includes(error?.code)
    || /AI 回傳內容不是有效 JSON|AI 回傳缺少 cues 陣列/.test(String(error?.message || ''));
}
...
function buildJsonRepairBody(body, batch, mode, language) {
  ...
  return {
    ...
    messages: [
      {
        role: 'system',
        content: `你是專業字幕處理器。${instruction}。上一個回應不是有效 JSON，或未符合必要的 JSON 結構；這次只能輸出一個 JSON object，禁止 Markdown、前後說明、註解或未跳脫換行。格式必須是 {"cues":[{"id":字幕原始 ID,"text":"字幕文字","reason":"簡短原因"}]}；保留每個 ID、數量與原順序。`,
      },
      {
        role: 'user',
