...
function buildSubtitleSchema(count) {
  return {
    type: 'json_schema',
    json_schema: {
        ...
        properties: {
          cues: {
            ...
            items: {
              ...
              properties: { id: { type: ['integer', 'string'] }, text: { type: 'string' }, reason: { type: 'string' } },
            },
          },
        },
      },
...
export async function optimizeSubtitleCues({ cues, config, mode = 'proofread', instructions = '', promptTemplate = '', glossary = [], language = 'zh-TW', signal, complete, checkpoint = {}, onCheckpoint }) {
  ...
  for (let batchIndex = startBatchIndex; batchIndex < totalBatches; batchIndex += 1) {
    ...
    const body = {
      ...
      ...(config.capabilities?.structuredOutput === 'json-schema' ? { response_format: buildSubtitleSchema(batch.length) } : {}),
      ...(config.capabilities?.jsonSchema !== false ? { response_format: { type: 'json_object' } } : {}),
      messages: [
        {
          role: 'system',
          content: `你是字幕校對員。${promptTemplate || MODE_INSTRUCTIONS[mode] || MODE_INSTRUCTIONS.proofread}${languagePromptInstruction(outputLanguage, mode)}不可更改 ID、順序、段落數或時間碼，不可杜撰內容。只輸出符合請求 schema 的 JSON。text 必須使用輸入字幕的實際文字；不可輸出「優化文字」、「原ID」、「簡短原因」等佔位文字。若不需修改，text 必須保留原文。`,
        },
        {
          role: 'user',
