...
使用 Small 轉錄時，App 會在寫入字幕前保留完整文字並整理過長 cue：正常情況每行最多 20 字元、每個 cue 最多兩行，超過 40 字元會依標點／空白拆成連續時間碼。若原始 cue 時間短到無法安全分配多段，會保留單一 cue，最多兩行且第二行可能超過 20 字元，以避免拆斷英文單字。這是可讀性處理，不會截斷或摘要內容；真實長音訊的斷句與閱讀速度仍請在校閱頁確認。
