...
- `.github/workflows/windows-preview.yml` 已對齊 0.51.0 並由 branch／tag push 觸發；既有 run `35700305138`、`35700315448`、`35714599428` 在 `test-breeze-asr.mjs` 的 Breeze 效能提示更新函式 assertion 失敗，修正後 run `35804611051` 已成功完成 source／FFmpeg regression 與後續 Windows preview packaging／renderer／archive steps。這不能視為 Windows 實機驗收，Windows scope 仍暫緩。
