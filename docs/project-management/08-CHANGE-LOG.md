...
- LM Studio 實測：LM Studio 0.4.20 已安裝並啟動 `127.0.0.1:1234/v1`；`qwen2.5-0.5b-instruct` 因無效 JSON 失敗，改用 `qwen2.5-1.5b-instruct` 後 1 cue 真實最佳化 completed、1/1 batch、0 retries、有效 JSON、0 changed cues。模型可用與端點探測通過，但繁體中文能力探測仍為 false，品質與真正斷網流程尚待驗收。
