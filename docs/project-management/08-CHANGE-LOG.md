...
- 風險與回復方式：所有外部驗收入口採固定來源、環境變數傳入金鑰、證據輸出遮罩；Windows PowerShell smoke 使用 ASCII-only script literals 避免 runner 編碼差異；若 Windows CI 或真實 endpoint 失敗，保留原始 log／JSON 並停止 Release 宣稱。
